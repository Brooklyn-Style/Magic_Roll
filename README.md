...
The `BGK` inner index column was set up to make `NG`, `RK`, and `BL` more comfortable. `D + BGK` are easily alt-fingerable. Magic allows me to put `M` and `B` on the same finger **and** two rows apart without any issue.
